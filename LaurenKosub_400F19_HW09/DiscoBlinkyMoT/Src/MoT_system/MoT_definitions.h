//MoT_definitions02.h wmh 2019-10-14 : updating MSG_post macro for new msgchannel design
//	note: RXBUF_.. and MSGHEAD, MSGTAIL definitions proably dont belong here
//  note: 'C' device versions of function sand data are not updated
// TODO  create macros to allocate structured data with allocations in the same place, so that that allocations match offsets to the struct elements ( 
//  this gave us a big headache when we had allocations and offsets separated (offsets were changing data in other structures when allcations were changed)
//MoT_definitions01.h wmh 2019-06-12 : collection of structure offsets, buffer sizes, etc using in the MoT design
...
//control and message channels --  structures used by USARTn interrupt.  Memory for the data structures are allocated in USARTn's code
	...
	//	.global USARTn_RXbufHANDLE //make visible in debugger and main()
	...
	//	.skip USARTn_RXBUFSIZE	//buffer data

	// constants defined  below are offsets from handle 'USART1_RXbufHANDLE' 'handle' of USART1_RXbufStruct allocated above.
	// USARTn_RXbufHANDLE is an argument of most calls to buffer operations defined here and elsewhere
	...
	#define  RXBUF0_RDINDEX -20		// word -- offset of current read location of BUF0 process
	#define  RXBUF1_RDINDEX -16		// ""                                         BUF1
...
// offsets of elements of 'msgchannel' message control structure
	#define MSGHEAD_PTR 0
	#define MSGTAIL_PTR 4
...
*/
	...
	@;------ wrapper for MSG_post_fn()  (see MoT_Msg_Handler.S)
		...
		@; 
		.macro MSG_post msgchannel, mydata, msgtext, msglength	@;MSG_post is called by commands and tasks to put a message in the message queue
			@; changes contents of r0-r3
			MOV_imm32 r3,\msgchannel	@; address of msg COMM function 
			MOV_imm32 r0,\mydata		@; address of this tasks device control block 
			ldr r1,[r0,#MSGLINK]		@; is a message from this task on the message list already ?
			cbnz r1,1f					@;		yes -  abort posting and report failure		
			MOV_imm32 r1,\msgtext		@;		else continue building message info
			str r1,[r0,#MSGPTR]			@; 		..
			MOV_imm32 r1,\msglength		@;  	..
			str r1,[r0,#MSGCOUNT]		@; 		..			
@;			push {r7,lr}				@;post the message !! QUESTION: if msgchannel_handler is a function then do we need to save r7,lr here? 
			blx r3						@; ..
@;			pop	{r7,lr}					@; ..				!! unnecessary?
			@;here after attempt to post with status in r0
			b 2f						@;continues af macro expansion with msgchannel_handler return status in r0
		1:	mov r0,#(-1)				@;continues with local failure (message not posted) status in r0
		...
		extern uint32_t USART3_msgctl;	//	because we haven't figured out a way to decorate the assembly language function's address	
